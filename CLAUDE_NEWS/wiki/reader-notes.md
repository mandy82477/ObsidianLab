...
- [⏳] 2026-07-29｜🔍 興趣主題｜使用者關心 **Codex 討論趨勢與「不同 agentic tool 配不同 model」的多模型混用走向**——觸發自 07-29 對話（源頭問題：Gemini/Claude/ChatGPT 三家適用場景比較 → 延伸到工具與模型解耦的未來格局）。**07-29 現場查證發現**：(a) `news/` 統計 Codex 提及總次數逐月上升（05 月 69 → 06 月 75 → 07 月 79，每日均次 2.3~2.9），質變重於量變——從 5 月下載量衝擊敘事（+1,397% vs CC −38%）演變為 7 月「何時選 Codex 而非 Claude」的分工比較文（SitePoint 07-25），社群已默認並存；(b) 「工具↔模型解耦」證據已在 wiki 累積成三層：廠商層（Claude Desktop 支援第三方 LLM、Workweave Router 嵌 CC/Codex/Cursor）、跨工具協作層（claude-anyteam 讓 Codex 加入 Agent Teams）、單任務多模型分工層（[[topics/community-pattern-trends]] 趨勢四「模型路由自動化」成形中，7/14 官方 46% 成本/96% 效能背書）。**現場判斷（推論）**：格局朝「編排層 vs 執行層分離」演化——旗艦模型搶編排者、便宜模型（DeepSeek/Kimi/GLM/Haiku）搶執行層，agentic tool 退化為可插拔容器，護城河從「工具綁模型」移至「誰當編排者」；反向訊號為黑盒路由靜默降階隱患（已載於 pattern-trends 頁）。**待 weekly-review 評估的提案**：「模型路由/多模型混用」目前散落三頁（[[topics/competitor-landscape]]、[[topics/community-pattern-trends]]、[[topics/community-tech-patterns]]），是否值得升格獨立 topic 頁聚合成一條主線（比照 07-22 large-codebase 前例），經使用者確認再建。
